...
/**
 * When this zone's HTML is served through the shell proxy (e.g. localhost:3000/player),
...
export default defineConfig(({ mode }) => {
	...
	const port = Number.parseInt(env.PORT ?? "3003", 10);
	...
	return {
		...
		plugins: [
			...
			tanstackStart({
				...
				// The shell proxy rule `/player/**` → 3003 covers this path,
				// routing RPC calls to the right zone server.
				serverFns: { base: "/player/_serverFn" },
			}),
			viteReact(),

	...
	return {
		...
		plugins: [
			...
			tanstackStart({
				...
				// The shell proxy rule `/tier-list/**` → 3002 covers this path,
				// routing RPC calls to the right zone server.
				serverFns: { base: "/tier-list/_serverFn" },
			}),
			viteReact(),
			...
			// RPC fetches land on the correct zone server instead of the shell.
			{
				...
				transform(code: string, id: string) {
					...
					return {
						code: code.replaceAll(/process\.env\.TSS_SERVER_FN_BASE/g, JSON.stringify("/tier-list/_serverFn/")),
					};
				},
			},
		],

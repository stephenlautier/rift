...
export default defineConfig({
	...
	rules: {
		...
		// ── Next.js plugin rules — off globally; scoped on in apps/next-* override ──
		// Adding the nextjs plugin enables all its rules; disable them globally so they
		// don't fire in Stencil components (libs/ui), Vike apps (apps/shell, mfe-*), etc.
	...
	overrides: [
		...
		{
			// Next.js app directory — enable nextjs plugin rules, disable Vike-era noise
			files: ["apps/next-*/**/*.ts", "apps/next-*/**/*.tsx"],
			rules: {
				// Re-enable nextjs rules (disabled globally to avoid Stencil/Vike false positives)
				...
				// Next.js App Router uses default exports for layout/page/loading/error files
				"import/no-default-export": "off",
				// Async Server Components have implicit return type — explicit is noise
				"typescript/explicit-function-return-type": "off",
				// App Router requires metadata const + default component in the same file
				"react/only-export-components": "off",
				// Server Actions must be defined inline in the async function — cannot be lifted
				"react-perf/jsx-no-new-function-as-prop": "off",
			},
		},
		{
			// libs/next-shared — peer dep of next, needs same relaxations as app code
			files: ["libs/next-shared/**/*.ts", "libs/next-shared/**/*.tsx"],
			rules: {
				"import/no-default-export": "off",
				"typescript/explicit-function-return-type": "off",
			},
